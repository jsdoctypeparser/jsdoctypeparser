...
/** @typedef {keyof typeof import('./NodeType')} NodeType */
/** @typedef {{ type: NodeType }} Node */

/**
 * Traverse the specified AST.
 * @param {Node} node AST to traverse.
 * @param {(node: Node) => void} [opt_onEnter] Callback for onEnter.
 * @param {(node: Node) => void} [opt_onLeave] Callback for onLeave.
 */
